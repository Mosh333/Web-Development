//Reuse accordion logic from Home Page 
                    ...
                    changeButton.addEventListener("click",function(event) {
                        ...
                        if(((contextQuesCount-numToDelete)>6)&&formFilled){ //Enough filled questions provided
                            ...
                            implementQuizSys[quizContextIndex].addEventListener("click", function(event) {
                                    ...
                                    console.log("How many times this is called?");
                                    var updateQuizMenuButtons = document.getElementsByClassName("quizMenu");
                                    for(i=0; i<updateQuizMenuButtons.length; i++){
                                    ...
                                    console.log("randomQuestionArray :", randomQuestionArray);
                                    //Insert Quiz Title
                                    var removeIdenticalQuizzes = document.getElementsByClassName("questionContainer");
                                    var removeIdenticalSubmitButtons = document.getElementsByClassName("submitQuiz");
                                    if(removeIdenticalQuizzes.length == 0 && removeIdenticalSubmitButtons.length == 0){
                                        var quizTitle = document.createElement("h2");
                                        ...
                                        //Since it recursively keeps adding more number of identical quiz,
                                        //just get the div container and the submit quiz, and delete
                                        //all the excessive ones
    /*                                     window.onload = function() {
                                            var removeIdenticalQuizzes = document.getElementsByClassName("questionContainer");
                                            var removeIdenticalSubmitButtons = document.getElementsByClassName("submitQuiz");
                                            console.log("Absolute New Test");
                                            if(removeIdenticalQuizzes.length > 7){
                                                for(i==removeIdenticalQuizzes.length; i>0; i--){
                                                    if(i>6){
                                                        removeIdenticalQuizzes[i].parentNode.removeChild(removeIdenticalQuizzes[i]);
                                                    }
                                                }
                                            }
                                            if(removeIdenticalSubmitButtons.length > 1){
                                                for(i = removeIdenticalSubmitButtons.length; i>0; i--){
                                                    if(i>0){
                                                        removeIdenticalSubmitButtons[i].parentNode.removeChild(removeIdenticalSubmitButtons[i]);
                                                    }
                                                }
                                            }
                                        };     */            
